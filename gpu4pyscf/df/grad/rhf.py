...
def get_jk(mf_grad, mol=None, dm0=None, hermi=0, with_j=True, with_k=True, omega=None):
    ...
    if with_j:
        ...
        vjaux_2c = gpunp.array([-vjaux[:,p0:p1].sum(axis=1) for p0, p1 in auxslices[:,2:]])
        rhoj = vjaux = tmp = None
    if with_k:
        nocc = orbo.shape[-1]
        if low.tag == 'eig':
        ...
        vkaux_2c = gpunp.array([-vkaux[:,p0:p1].sum(axis=1) for p0, p1 in auxslices[:,2:]])
        vkaux = tmp = None
        if not auxmol.cart:
    ...
    aux2atom = gpunp.asarray(aux2atom)
    if with_j:
