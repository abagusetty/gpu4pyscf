...
def _jk_ip_task(intopt, rhoj_cart, dm_cart, rhok_cart, orbo_cart, task_list,
    ...
    orbo_cart = gpunp.asarray(orbo_cart)
    cart_aux_loc = intopt.cart_aux_loc
    nao_cart = dm_cart.shape[0]
    naux_cart = intopt._sorted_auxmol.nao
    vj = vk = vjaux = vkaux = None
    if with_j:
        rhoj_cart = gpunp.asarray(rhoj_cart)
        dm_cart = gpunp.asarray(dm_cart)
        vj = gpunp.zeros((3,nao_cart), order='C')
        vjaux = gpunp.zeros((3,naux_cart))
    if with_k:
        rhok_cart = gpunp.asarray(rhok_cart)
        vk = gpunp.zeros((3,nao_cart), order='C')
        vkaux = gpunp.zeros((3,naux_cart))
    ...
    for cp_kl_id in task_list:
        ...
        if(rhoj_tmp.flags['C_CONTIGUOUS'] == False):
            rhoj_tmp = rhoj_tmp.astype(gpunp.float64, order='C')
