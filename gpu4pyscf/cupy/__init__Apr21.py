...
# CRITICAL: dpnp.asarray and dpnp.array were bound to cupy_fake BEFORE
# cuda.py ran _patch_dpnp_creation_apis(). The patch replaced them on the
# dpnp module with _wrap_with_master_queue wrappers, but cupy_fake still
# holds the original __wrapped__ references. Rebind explicitly NOW, after
# patching is guaranteed complete.
cupy_fake.asarray = dpnp.asarray
cupy_fake.array   = dpnp.array

# Refresh direct aliases: cuda.py monkey-patched dpnp creation functions
# in-place to inject sycl_queue=master_queue. The early `cupy_fake.X = dpnp.X`
# bindings at the top of this file captured the PRE-patch references and
# are now stale — refresh them here so cupy.foo() reaches the patched
# version and every allocation lands on the master queue.
#
# Skip names we've replaced (or will replace) with custom cupy_fake shims:
# those shims already call the now-patched dpnp.* underneath, so they
# inherit queue injection without needing a direct rebind.
...
for _attr in dir(dpnp):
    if _attr.startswith("_"):
        continue
    if _attr in _CUSTOM_CUPY_FAKE_SHIMS:
        continue
...
def _safe_setitem(self, key, value):
    """Mirror of _safe_getitem for assignment. Coerces slice bounds that
    are 0-D / length-1 integer arrays (CuPy-permissive, dpctl-strict)
    while leaving top-level fancy-index arrays untouched."""
    if _original_setitem is None:
        raise AttributeError("__setitem__ not found on dpnp.ndarray")
...
dpnp.ndarray.__setitem__ = _safe_setitem
...
def _coerce_index_scalar(x):
    """CuPy silently accepts 0-D and length-1 integer arrays as scalar
    indices/slice bounds; dpctl rejects them. Unwrap to a Python int so
    code written against cupy works unchanged under dpnp."""
    if x is None or isinstance(x, (bool, int)):
        return x
    if isinstance(x, (np.integer, np.bool_)):
        return int(x)
    if isinstance(x, (np.ndarray, dpnp.ndarray)) \
            and x.ndim <= 1 and x.size == 1 \
            and x.dtype.kind in ("b", "i", "u"):
        return int(x)
    return x

def _fix_slice(s):
    return slice(_coerce_index_scalar(s.start),
                 _coerce_index_scalar(s.stop),
                 _coerce_index_scalar(s.step))

def _fix_key(k):
    if isinstance(k, slice):
        return _fix_slice(k)
    # NOTE: deliberately do NOT call _coerce_index_scalar here — top-level
    # array keys are fancy indices, where length-1 ≠ scalar.
    if isinstance(k, (np.integer, np.bool_)):
        return int(k)
    return _to_device_index(k)

def _safe_getitem(self, key):
    if _original_getitem is None:
        raise AttributeError("__getitem__ not found on dpnp.ndarray")
    if isinstance(key, tuple):
        return _original_getitem(self, tuple(_fix_key(k) for k in key))
    return _original_getitem(self, _fix_key(key))
...
##########################################################################
# ── Memory pool — reports actual SYCL device memory via cuda module ──
#
# FIX: previously _DummyMemoryPool was defined twice. The first all-zeros
# definition created _dummy_pool and was registered on cupy_fake BEFORE
# _setup_cupy_backends(). The second real definition (below) redefined the
# class name but never updated _dummy_pool or re-registered on cupy_fake,
# so get_default_memory_pool() always returned used_bytes=0.
#
# Fix: single definition here, after _setup_cupy_backends(), with a fresh
# singleton and correct registration on cupy_fake.
##########################################################################

class _MemoryPool:
    """Reports actual SYCL device memory usage via the cuda shim module.

    used_bytes  = total − free   (bytes currently occupied on device)
    free_bytes  = free memory    (bytes available)
...
# Single singleton — created AFTER _MemoryPool is fully defined.
...
def _get_default_pinned_memory_pool():
    """Pinned memory has no equivalent under SYCL/dpnp; return same pool."""
    return _memory_pool

# Register on cupy_fake — overrides any earlier binding.
...
# cupy.cuda.PinnedMemoryPool shim
if hasattr(cupy_fake, 'cuda'):
    cupy_fake.cuda.PinnedMemoryPool = _MemoryPool
    
##########################################################################

def fuse(*args, **kwargs):
    """No-op replacement for cupy.fuse under dpnp.

    cupy.fuse can be used two ways:
      @cupy.fuse
      def f(x): ...
    ...
    Both forms must return something callable that, when called with the
    user's function, returns that function unchanged.
    """
    # Form 1: used as @cupy.fuse  (no parens) — args == (func,)
    if len(args) == 1 and callable(args[0]) and not kwargs:
        return args[0]
    # Form 2: used as @cupy.fuse(...) — return a decorator
    def _decorator(func):
        return func
    return _decorator
