...
template <int L> __device__ static
void _dm_to_dm_xyz_derivx(double *cache, double *dm_xyz, double *dm, int nao, int li, int lj,
...
template <int L> __device__ static
void _dm_to_dm_xyz_derivy(double *cache, double *dm_xyz, double *dm, int nao, int li, int lj,
                          double *ri, double *rj, double ai2, double aj2,
                          double cicj, int npairs_per_block)
{
#ifdef USE_SYCL
...
template <int L> __device__ static
void _dm_to_dm_xyz_derivz(double *cache, double *dm_xyz, double *dm, int nao, int li, int lj,
                          double *ri, double *rj, double ai2, double aj2,
                          double cicj, int npairs_per_block)
{
#ifdef USE_SYCL
                           ...
                           MGridBounds bounds, double *pool, uint32_t pair_idx0)
{
    ...
    // dx * dx
    _dm_to_dm_xyz_derivx<L>(cache, dm_xyz, dm+i0*nao+j0, nao, li, lj, ri, rj, ai2, aj2,
                            cicj, npairs_this_block);
    __syncthreads();
    fill_gx_dmyz<L>(cache, gx_dmyz, dm_xyz, xs_exp, ngridx, ngrid_span, npairs_this_block);
...
#pragma unroll
    ...
    _dm_to_dm_xyz_derivy<L>(cache, dm_xyz, dm+i0*nao+j0, nao, li, lj, ri, rj, ai2, aj2,
                            cicj, npairs_this_block);
    __syncthreads();
    fill_gx_dmyz<L>(cache, gy_dmxz, dm_xyz, ys_exp, ngridy, ngrid_span, npairs_this_block);
...
#pragma unroll
    ...
    _dm_to_dm_xyz_derivz<L>(cache, dm_xyz, dm+i0*nao+j0, nao, li, lj, ri, rj, ai2, aj2,
                            cicj, npairs_this_block);
    __syncthreads();
    fill_gx_dmyz<L>(cache, gz_dmxy, dm_xyz, zs_exp, ngridz, ngrid_span, npairs_this_block);
    for (int sp_id = 0; sp_id < npairs_this_block; ++sp_id) {
        int nx0 = grid_start[0*WARP_SIZE+sp_id];
        int ny0 = grid_start[1*WARP_SIZE+sp_id];
        int nz0 = grid_start[2*WARP_SIZE+sp_id];
                          ...
                          )
{
    ...
    while (pair_idx0 < bounds.nshl_pair) {
      _eval_tau_orth_kernel<L>(cache, rho, dm, envs, bounds, pool, pair_idx0);
        if (thread_id == 0) {
            pair_idx0 = atomicAdd(batch_head, WARP_SIZE);
        }
        __syncthreads();
                     ...
                     int *bas_ij_idx, double *pool, int workers)
{
    ...
    switch (l) {
    case 0: stream.submit([&](sycl::handler &cgh) { sycl::local_accessor<double, 1> local_acc(sycl::range<1>(buflen(0, &bounds)), cgh); cgh.parallel_for(sycl::nd_range<1>(workers * THREADS, THREADS), [=](auto item) { eval_tau_orth_kernel<0> (rho, dm, envs, bounds, pool, batch_head, item, GPU4PYSCF_IMPL_SYCL_GET_MULTI_PTR(local_acc)); }); }); break;
    case 1: stream.submit([&](sycl::handler &cgh) { sycl::local_accessor<double, 1> local_acc(sycl::range<1>(buflen(1, &bounds)), cgh); cgh.parallel_for(sycl::nd_range<1>(workers * THREADS, THREADS), [=](auto item) { eval_tau_orth_kernel<1> (rho, dm, envs, bounds, pool, batch_head, item, GPU4PYSCF_IMPL_SYCL_GET_MULTI_PTR(local_acc)); }); }); break;
    case 2: stream.submit([&](sycl::handler &cgh) { sycl::local_accessor<double, 1> local_acc(sycl::range<1>(buflen(2, &bounds)), cgh); cgh.parallel_for(sycl::nd_range<1>(workers * THREADS, THREADS), [=](auto item) { eval_tau_orth_kernel<2> (rho, dm, envs, bounds, pool, batch_head, item, GPU4PYSCF_IMPL_SYCL_GET_MULTI_PTR(local_acc)); }); }); break;
    case 3: stream.submit([&](sycl::handler &cgh) { sycl::local_accessor<double, 1> local_acc(sycl::range<1>(buflen(3, &bounds)), cgh); cgh.parallel_for(sycl::nd_range<1>(workers * THREADS, THREADS), [=](auto item) { eval_tau_orth_kernel<3> (rho, dm, envs, bounds, pool, batch_head, item, GPU4PYSCF_IMPL_SYCL_GET_MULTI_PTR(local_acc)); }); }); break;
    case 4: stream.submit([&](sycl::handler &cgh) { sycl::local_accessor<double, 1> local_acc(sycl::range<1>(buflen(4, &bounds)), cgh); cgh.parallel_for(sycl::nd_range<1>(workers * THREADS, THREADS), [=](auto item) { eval_tau_orth_kernel<4> (rho, dm, envs, bounds, pool, batch_head, item, GPU4PYSCF_IMPL_SYCL_GET_MULTI_PTR(local_acc)); }); }); break;
    case 5: stream.submit([&](sycl::handler &cgh) { sycl::local_accessor<double, 1> local_acc(sycl::range<1>(buflen(5, &bounds)), cgh); cgh.parallel_for(sycl::nd_range<1>(workers * THREADS, THREADS), [=](auto item) { eval_tau_orth_kernel<5> (rho, dm, envs, bounds, pool, batch_head, item, GPU4PYSCF_IMPL_SYCL_GET_MULTI_PTR(local_acc)); }); }); break;
    case 6: stream.submit([&](sycl::handler &cgh) { sycl::local_accessor<double, 1> local_acc(sycl::range<1>(buflen(6, &bounds)), cgh); cgh.parallel_for(sycl::nd_range<1>(workers * THREADS, THREADS), [=](auto item) { eval_tau_orth_kernel<6> (rho, dm, envs, bounds, pool, batch_head, item, GPU4PYSCF_IMPL_SYCL_GET_MULTI_PTR(local_acc)); }); }); break;
    case 7: stream.submit([&](sycl::handler &cgh) { sycl::local_accessor<double, 1> local_acc(sycl::range<1>(buflen(7, &bounds)), cgh); cgh.parallel_for(sycl::nd_range<1>(workers * THREADS, THREADS), [=](auto item) { eval_tau_orth_kernel<7> (rho, dm, envs, bounds, pool, batch_head, item, GPU4PYSCF_IMPL_SYCL_GET_MULTI_PTR(local_acc)); }); }); break;
    case 8: stream.submit([&](sycl::handler &cgh) { sycl::local_accessor<double, 1> local_acc(sycl::range<1>(buflen(8, &bounds)), cgh); cgh.parallel_for(sycl::nd_range<1>(workers * THREADS, THREADS), [=](auto item) { eval_tau_orth_kernel<8> (rho, dm, envs, bounds, pool, batch_head, item, GPU4PYSCF_IMPL_SYCL_GET_MULTI_PTR(local_acc)); }); }); break;
    default: return 1;
    }
