                     ...
                     int *bas_ij_idx, double *pool, int workers)
{
    ...
    switch (l) {
    case 0: stream.submit([&](sycl::handler &cgh) { sycl::local_accessor<double, 1> local_acc(sycl::range<1>(buflen(0, &bounds)), cgh); cgh.parallel_for<class eval_tau_orth_kernel_0_sycl>(sycl::nd_range<1>(workers * THREADS, THREADS), [=](auto item) { eval_tau_orth_kernel<0> (rho, dm, envs, bounds, pool, batch_head, item, GPU4PYSCF_IMPL_SYCL_GET_MULTI_PTR(local_acc)); }); }); break;
    case 1: stream.submit([&](sycl::handler &cgh) { sycl::local_accessor<double, 1> local_acc(sycl::range<1>(buflen(1, &bounds)), cgh); cgh.parallel_for<class eval_tau_orth_kernel_1_sycl>(sycl::nd_range<1>(workers * THREADS, THREADS), [=](auto item) { eval_tau_orth_kernel<1> (rho, dm, envs, bounds, pool, batch_head, item, GPU4PYSCF_IMPL_SYCL_GET_MULTI_PTR(local_acc)); }); }); break;
    case 2: stream.submit([&](sycl::handler &cgh) { sycl::local_accessor<double, 1> local_acc(sycl::range<1>(buflen(2, &bounds)), cgh); cgh.parallel_for<class eval_tau_orth_kernel_2_sycl>(sycl::nd_range<1>(workers * THREADS, THREADS), [=](auto item) { eval_tau_orth_kernel<2> (rho, dm, envs, bounds, pool, batch_head, item, GPU4PYSCF_IMPL_SYCL_GET_MULTI_PTR(local_acc)); }); }); break;
    case 3: stream.submit([&](sycl::handler &cgh) { sycl::local_accessor<double, 1> local_acc(sycl::range<1>(buflen(3, &bounds)), cgh); cgh.parallel_for<class eval_tau_orth_kernel_3_sycl>(sycl::nd_range<1>(workers * THREADS, THREADS), [=](auto item) { eval_tau_orth_kernel<3> (rho, dm, envs, bounds, pool, batch_head, item, GPU4PYSCF_IMPL_SYCL_GET_MULTI_PTR(local_acc)); }); }); break;
    case 4: stream.submit([&](sycl::handler &cgh) { sycl::local_accessor<double, 1> local_acc(sycl::range<1>(buflen(4, &bounds)), cgh); cgh.parallel_for<class eval_tau_orth_kernel_4_sycl>(sycl::nd_range<1>(workers * THREADS, THREADS), [=](auto item) { eval_tau_orth_kernel<4> (rho, dm, envs, bounds, pool, batch_head, item, GPU4PYSCF_IMPL_SYCL_GET_MULTI_PTR(local_acc)); }); }); break;
    case 5: stream.submit([&](sycl::handler &cgh) { sycl::local_accessor<double, 1> local_acc(sycl::range<1>(buflen(5, &bounds)), cgh); cgh.parallel_for<class eval_tau_orth_kernel_5_sycl>(sycl::nd_range<1>(workers * THREADS, THREADS), [=](auto item) { eval_tau_orth_kernel<5> (rho, dm, envs, bounds, pool, batch_head, item, GPU4PYSCF_IMPL_SYCL_GET_MULTI_PTR(local_acc)); }); }); break;
    case 6: stream.submit([&](sycl::handler &cgh) { sycl::local_accessor<double, 1> local_acc(sycl::range<1>(buflen(6, &bounds)), cgh); cgh.parallel_for<class eval_tau_orth_kernel_6_sycl>(sycl::nd_range<1>(workers * THREADS, THREADS), [=](auto item) { eval_tau_orth_kernel<6> (rho, dm, envs, bounds, pool, batch_head, item, GPU4PYSCF_IMPL_SYCL_GET_MULTI_PTR(local_acc)); }); }); break;
    case 7: stream.submit([&](sycl::handler &cgh) { sycl::local_accessor<double, 1> local_acc(sycl::range<1>(buflen(7, &bounds)), cgh); cgh.parallel_for<class eval_tau_orth_kernel_7_sycl>(sycl::nd_range<1>(workers * THREADS, THREADS), [=](auto item) { eval_tau_orth_kernel<7> (rho, dm, envs, bounds, pool, batch_head, item, GPU4PYSCF_IMPL_SYCL_GET_MULTI_PTR(local_acc)); }); }); break;
    case 8: stream.submit([&](sycl::handler &cgh) { sycl::local_accessor<double, 1> local_acc(sycl::range<1>(buflen(8, &bounds)), cgh); cgh.parallel_for<class eval_tau_orth_kernel_8_sycl>(sycl::nd_range<1>(workers * THREADS, THREADS), [=](auto item) { eval_tau_orth_kernel<8> (rho, dm, envs, bounds, pool, batch_head, item, GPU4PYSCF_IMPL_SYCL_GET_MULTI_PTR(local_acc)); }); }); break;
    default: return 1;
    }
