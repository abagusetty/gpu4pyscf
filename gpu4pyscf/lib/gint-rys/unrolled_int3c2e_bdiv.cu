...
// Note: Here shared memory `rw_buffer` is passed as a parameter
// over dynamic-shared memory declaration is to unify the interfaces
// between SYCL (has limitations related to dynamic SM) & CUDA
__device__ inline
