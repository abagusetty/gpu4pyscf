...
#if USE_SYCL
#define atomicOr(addr, val) (sycl::atomic_ref<int, \
                                         sycl::memory_order::relaxed, \
                                         sycl::memory_scope::device, \
                                         sycl::access::address_space::global_space>(*(addr)).fetch_or(val))
//  auto atomic_ref = sycl::atomic_ref<int, 
//                                        sycl::memory_order::relaxed, 
//                                        sycl::memory_scope::device, 
//                                        sycl::access::address_space::global_space>(acc[idx]);
...
template <int ANG> __attribute__((always_inline))
...
void _screen_index(int *non0shl_idx, double cutoff, int l, int ish, int nprim, double *coords, int ngrids, sycl::nd_item<1>& item){
    int grid_id = static_cast<int>(item.get_global_id(1));
    if (grid_id >= ngrids){
        return;
    }
    int natm = c_envs.get().natm;
    int atm_id = c_bas_atom[ish];
    double* atm_coords = c_envs.get().atom_coordx;
    ...
    double *exps = c_envs.get().env + c_bas_exp[ish];
    double *coeffs = c_envs.get().env + c_bas_coeff[ish];
    double maxc = 0.0;
    double min_exp = 1e9;
    for (int ip = 0; ip < nprim; ++ip) {
        min_exp = MIN(min_exp, exps[ip]);
        maxc = MAX(maxc, std::fabs(coeffs[ip]));
    }
    double gto_sup = -min_exp * rr + .5 * sycl::log(rr) * l + sycl::log(maxc);
    int is_large = gto_sup > sycl::log(cutoff);
...
template <int ANG> __attribute__((always_inline))
static void _cart_kernel_deriv0(BasOffsets offsets, sycl::nd_item<2>& item)
{
    ...
    int bas_id = static_cast<int>(item.get_group(0));
    int natm = c_envs.get().natm;
    ...
    double *atom_coordx = c_envs.get().atom_coordx;
    double *atom_coordy = c_envs.get().atom_coordx + natm;
    double *atom_coordz = c_envs.get().atom_coordx + natm * 2;
    ...
    double rr = rx * rx + ry * ry + rz * rz;
    double *exps = c_envs.get().env + c_bas_exp[glob_ish];
    double *coeffs = c_envs.get().env + c_bas_coeff[glob_ish];

    double ce = 0;
    for (int ip = 0; ip < offsets.nprim; ++ip) {
        ce += coeffs[ip] * sycl::exp(-exps[ip] * rr);
    }
    ce *= offsets.fac;
...
template <int ANG> __attribute__((always_inline))
static void _cart_kernel_deriv1(BasOffsets offsets, sycl::nd_item<2>& item)
{
    int ngrids = offsets.ngrids;
    int grid_id = static_cast<int>(item.get_global_id(1));
    if (grid_id >= ngrids) {
    ...
    if (ANG == 0) {
        ...
        for (int ip = 0; ip < offsets.nprim; ++ip) {
            double ce = coeffs[ip] * sycl::exp(-exps[ip] * rr) * offsets.fac;
...
template <int ANG> __attribute__((always_inline))
static void _cart_kernel_deriv2(BasOffsets offsets, sycl::nd_item<2>& item)
{
    int ngrids = offsets.ngrids;
    int grid_id = static_cast<int>(item.get_global_id(1));
    if (grid_id >= ngrids) {
...
template <int ANG> __attribute__((always_inline))
static void _cart_kernel_deriv3(BasOffsets offsets, sycl::nd_item<2>& item)
{
    int ngrids = offsets.ngrids;
    int grid_id = static_cast<int>(item.get_global_id(1));
    if (grid_id >= ngrids) {
...
template <int ANG> __attribute__((always_inline))
static void _cart_kernel_deriv4(BasOffsets offsets, sycl::nd_item<2>& item)
{
    int ngrids = offsets.ngrids;
    int grid_id = static_cast<int>(item.get_global_id(1));
    if (grid_id >= ngrids) {
...
template <int ANG> __attribute__((always_inline))
static void _sph_kernel_deriv0(BasOffsets offsets, sycl::nd_item<2>& item)
{
    int ngrids = offsets.ngrids;
    int grid_id = static_cast<int>(item.get_global_id(1));
    if (grid_id >= ngrids) {
...
template <int ANG> __attribute__((always_inline))
static void _sph_kernel_deriv1(BasOffsets offsets, sycl::nd_item<2>& item)
{
    int ngrids = offsets.ngrids;
    int grid_id = static_cast<int>(item.get_global_id(1));
    if (grid_id >= ngrids) {
...
template <int ANG> __attribute__((always_inline))
static void _sph_kernel_deriv2(BasOffsets offsets, sycl::nd_item<2>& item)
{
    int ngrids = offsets.ngrids;
    int grid_id = static_cast<int>(item.get_global_id(1));
    if (grid_id >= ngrids) {
...
template <int ANG> __attribute__((always_inline))
static void _sph_kernel_deriv3(BasOffsets offsets, sycl::nd_item<2>& item)
{
    int ngrids = offsets.ngrids;
    int grid_id = static_cast<int>(item.get_global_id(1));
    if (grid_id >= ngrids) {
...
template <int ANG> __attribute__((always_inline))
static void _sph_kernel_deriv4(BasOffsets offsets, sycl::nd_item<2>& item)
{
    int ngrids = offsets.ngrids;
    int grid_id = static_cast<int>(item.get_global_id(1));
    if (grid_id >= ngrids) {
...
extern "C" {
// __host__
void GDFTinit_envs(sycl::queue stream, GTOValEnvVars **envs_cache, int *ao_loc,
                   int *atm, int natm, int *bas, int nbas, double *env, int nenv)
{
    ...
    *envs_cache = envs;
    ...
    sycl_get_queue()->memcpy(c_envs, envs, sizeof(GTOValEnvVars));
    sycl_get_queue()->memcpy(c_bas_atom, bas_atom, sizeof(uint16_t)*NBAS_MAX);
    sycl_get_queue()->memcpy(c_bas_exp, bas_exp, sizeof(uint16_t)*NBAS_MAX);
    sycl_get_queue()->memcpy(c_bas_coeff, bas_coeff, sizeof(uint16_t)*NBAS_MAX);
    sycl_get_queue()->wait();
}
...
int GDFTeval_gto(sycl::queue& stream, double *ao, int deriv, int cart,
                 ...
                 int *bas)
{
    ...
    for (int ictr = 0; ictr < nctr; ++ictr) {
        ...
        case 0:
            if (cart == 1) {
                switch (l) {
                case 0: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv0<0>(offsets, item); }); break;
                case 1: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv0<1>(offsets, item); }); break;
                case 2: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv0<2>(offsets, item); }); break;
                case 3: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv0<3>(offsets, item); }); break;
                case 4: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv0<4>(offsets, item); }); break;
                case 5: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv0<5>(offsets, item); }); break;
                case 6: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv0<6>(offsets, item); }); break;
                case 7: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv0<7>(offsets, item); }); break;
                case 8: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv0<8>(offsets, item); }); break;
                default:fprintf(stderr, "l = %d not supported\n", l); }
            } else {
                switch (l) {
                case 0: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv0<0>(offsets, item); }); break;
                case 1: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv0<1>(offsets, item); }); break;
                case 2: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv0 <2>(offsets, item); }); break;
                case 3: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv0 <3>(offsets, item); }); break;
                case 4: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv0 <4>(offsets, item); }); break;
                case 5: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv0 <5>(offsets, item); }); break;
                case 6: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv0 <6>(offsets, item); }); break;
                case 7: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv0 <7>(offsets, item); }); break;
                case 8: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv0 <8>(offsets, item); }); break;
                default: fprintf(stderr, "l = %d not supported\n", l); }
            }
            break;
        case 1:
            if (cart == 1) {
                switch (l) {
                case 0: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv1<0>(offsets, item); }); break;
                case 1: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv1<1>(offsets, item); }); break;
                case 2: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv1<2>(offsets, item); }); break;
                case 3: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv1<3>(offsets, item); }); break;
                case 4: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv1<4>(offsets, item); }); break;
                case 5: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv1<5>(offsets, item); }); break;
                case 6: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv1<6>(offsets, item); }); break;
                case 7: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv1<7>(offsets, item); }); break;
                case 8: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv1<8>(offsets, item); }); break;
                default: fprintf(stderr, "l = %d not supported\n", l); }
            } else {
                switch (l) {
                case 0: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv1<0>(offsets, item); }); break;
                case 1: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv1<1>(offsets, item); }); break;
                case 2: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv1 <2>(offsets, item); }); break;
                case 3: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv1 <3>(offsets, item); }); break;
                case 4: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv1 <4>(offsets, item); }); break;
                case 5: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv1 <5>(offsets, item); }); break;
                case 6: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv1 <6>(offsets, item); }); break;
                case 7: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv1 <7>(offsets, item); }); break;
                case 8: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv1 <8>(offsets, item); }); break;
                default: fprintf(stderr, "l = %d not supported\n", l); }
            }
            break;
        case 2:
            if (cart == 1){
                switch (l) {
                case 0: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv2<0>(offsets, item); }); break;
                case 1: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv2<1>(offsets, item); }); break;
                case 2: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv2<2>(offsets, item); }); break;
                case 3: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv2<3>(offsets, item); }); break;
                case 4: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv2<4>(offsets, item); }); break;
                case 5: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv2<5>(offsets, item); }); break;
                case 6: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv2<6>(offsets, item); }); break;
                case 7: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv2<7>(offsets, item); }); break;
                case 8: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv2<8>(offsets, item); }); break;
                default: fprintf(stderr, "l = %d not supported\n", l); break;}
            } else {
                switch(l){
                case 0: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv2<0>(offsets, item); }); break;
                case 1: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv2<1>(offsets, item); }); break;
                case 2: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv2<2>(offsets, item); }); break;
                case 3: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv2<3>(offsets, item); }); break;
                case 4: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv2<4>(offsets, item); }); break;
                case 5: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv2<5>(offsets, item); }); break;
                case 6: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv2<6>(offsets, item); }); break;
                case 7: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv2<7>(offsets, item); }); break;
                case 8: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv2<8>(offsets, item); }); break;
                default: fprintf(stderr, "l = %d not supported\n", l); break; }
                }
            break;
        case 3:
            if (cart == 1){
                switch (l) {
                case 0: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv3<0>(offsets, item); }); break;
                case 1: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv3<1>(offsets, item); }); break;
                case 2: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv3<2>(offsets, item); }); break;
                case 3: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv3<3>(offsets, item); }); break;
                case 4: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv3<4>(offsets, item); }); break;
                case 5: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv3<5>(offsets, item); }); break;
                case 6: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv3<6>(offsets, item); }); break;
                case 7: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv3<7>(offsets, item); }); break;
                case 8: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv3<8>(offsets, item); }); break;
                default: fprintf(stderr, "l = %d not supported\n", l); break; }
            } else {
                switch(l){
                case 0: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv3<0>(offsets, item); }); break;
                case 1: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv3<1>(offsets, item); }); break;
                case 2: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv3<2>(offsets, item); }); break;
                case 3: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv3<3>(offsets, item); }); break;
                case 4: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv3<4>(offsets, item); }); break;
                case 5: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv3<5>(offsets, item); }); break;
                case 6: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv3<6>(offsets, item); }); break;
                case 7: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv3<7>(offsets, item); }); break;
                case 8: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv3<8>(offsets, item); }); break;
                default: fprintf(stderr, "l = %d not supported\n", l); break; }
                }
            break;
        case 4:
            if (cart == 1){
                switch (l) {
                case 0: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv4<0>(offsets, item); }); break;
                case 1: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv4<1>(offsets, item); }); break;
                case 2: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv4<2>(offsets, item); }); break;
                case 3: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv4<3>(offsets, item); }); break;
                case 4: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv4<4>(offsets, item); }); break;
                case 5: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv4<5>(offsets, item); }); break;
                case 6: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv4<6>(offsets, item); }); break;
                case 7: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv4<7>(offsets, item); }); break;
                case 8: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv4<8>(offsets, item); }); break;
                default: fprintf(stderr, "l = %d not supported\n", l); break; }
            } else {
                switch(l){
                case 0: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv4<0>(offsets, item); }); break;
                case 1: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _cart_kernel_deriv4<1>(offsets, item); }); break;
                case 2: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv4<2>(offsets, item); }); break;
                case 3: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv4<3>(offsets, item); }); break;
                case 4: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv4<4>(offsets, item); }); break;
                case 5: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv4<5>(offsets, item); }); break;
                case 6: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv4<6>(offsets, item); }); break;
                case 7: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv4<7>(offsets, item); }); break;
                case 8: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { _sph_kernel_deriv4<8>(offsets, item); }); break;
                default: fprintf(stderr, "l = %d not supported\n", l); break; }
            }
            break;
        default:
...
int GDFTscreen_index(sycl::queue& stream, int *non0shl_idx, double cutoff,
		     double *grids, int ngrids, int *bas_loc, int nbas, int *bas)
{
	...
	stream.parallel_for(sycl::nd_range<1>(blocks * threads, threads), [=](auto item) {
	    _screen_index(non0shl_idx, cutoff, l, shl_id, nprim, grids, ngrids, item);
	});
    }
    return 0;
}
