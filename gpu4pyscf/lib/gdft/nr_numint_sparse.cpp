...
static void _dot_ao_dm(double *out, double *ao, double *dm, int jsh0, int jsh1,
                       ...
                       uint8_t *screen_index, uint8_t *pair_mask, int *ao_loc, sycl::nd_item<3>& item)
{
    int tx = static_cast<int>(item.get_local_id(2));
    int ty = static_cast<int>(item.get_local_id(1));
    int grid_blk = static_cast<int>(item.get_group(2));
    int shell_blk = static_cast<int>(item.get_group(1));
...
static void _dot_aow_ao(double *out, double *bra, double *ket, double *wv,
                        ...
                        int *bas_pair2bra, int *bas_pair2ket, int *ao_loc, sycl::nd_item<3>& item)
{
    int task_ij = static_cast<int>(item.get_group(2));
    int tx = static_cast<int>(item.get_local_id(2));
    int ty = static_cast<int>(item.get_local_id(1));
    int tz = static_cast<int>(item.get_local_id(0));
    int txy = ty * DIVXY + tx;
...
//{
//    int grid_blk = static_cast<int>(item.get_group(2));
//    int grid_id = grid_blk * THREADSX + static_cast<int>(item.get_local_id(2));
//
//    double val = 0;
