 ...
 *
 * ExchCXX's device entry points take a backend-specific stream handle
 * (DEVICE_PARAMS in exchcxx_macros.hpp): `cudaStream_t` for the CUDA build,
 * `sycl::queue*` for the SYCL build. The libxc-ABI entry points below take an
 * opaque `void*` from Python either way, so everything backend-specific is
 * confined to the three helpers here.
 * ---------------------------------------------------------------------- */
#if defined(EXCHCXX_ENABLE_SYCL)

using gdft_stream_t = sycl::queue*;

static inline gdft_stream_t gdft_stream_cast(void* s) {
  return reinterpret_cast<sycl::queue*>(s);
}
static inline void gdft_memset_async(gdft_stream_t s, void* p, int v, std::size_t n) {
  s->memset(p, v, n);
}
static inline void gdft_stream_sync(gdft_stream_t s) { s->wait(); }
static inline void gdft_memcpy_d2h(gdft_stream_t s, void* dst, const void* src, std::size_t n) {
  s->memcpy(dst, src, n);
}

#elif defined(EXCHCXX_ENABLE_CUDA)

using gdft_stream_t = cudaStream_t;

static inline gdft_stream_t gdft_stream_cast(void* s) {
  return static_cast<cudaStream_t>(s);
}
static inline void gdft_memset_async(gdft_stream_t s, void* p, int v, std::size_t n) {
  cudaMemsetAsync(p, v, n, s);
}
static inline void gdft_stream_sync(gdft_stream_t s) { cudaStreamSynchronize(s); }
static inline void gdft_memcpy_d2h(gdft_stream_t s, void* dst, const void* src, std::size_t n) {
  cudaMemcpyAsync(dst, src, n, cudaMemcpyDeviceToHost, s);
}

#else
  #error "exchcxx.cpp requires ExchCXX built with either CUDA or SYCL device support"
#endif

/* Init-time breadcrumbs are off unless GDFT_EXCHCXX_VERBOSE is set. */
static bool exchcxx_verbose() {
  static const bool v = (std::getenv("GDFT_EXCHCXX_VERBOSE") != nullptr);
  return v;
}
...
#if defined(EXCHCXX_ENABLE_CUDA)
static const char* kVers  = "ExchCXX-CUDA 1.0";
#else
...
static inline void zero_gga_out(
  ...
  if(order >= 0) gdft_memset_async(q, out->zk, 0, sizeof(double)*np*func->dim.zk);
  if(order >= 1) {
    gdft_memset_async(q, out->vrho, 0, sizeof(double)*np*func->dim.vrho);
    gdft_memset_async(q, out->vsigma, 0, sizeof(double)*np*func->dim.vsigma); // (sigma, lapl, tau)
  }
  if(order >= 2) {
    gdft_memset_async(q, out->v2rho2, 0, sizeof(double)*np*func->dim.v2rho2);
    gdft_memset_async(q, out->v2rhosigma, 0, sizeof(double)*np*func->dim.v2rhosigma);
    gdft_memset_async(q, out->v2sigma2, 0, sizeof(double)*np*func->dim.v2sigma2);
  }
  if(order >= 3) {
    gdft_memset_async(q, out->v3rho3,       0, sizeof(double)*np*func->dim.v3rho3);
    gdft_memset_async(q, out->v3rho2sigma,  0, sizeof(double)*np*func->dim.v3rho2sigma);
    gdft_memset_async(q, out->v3rhosigma2,  0, sizeof(double)*np*func->dim.v3rhosigma2);
    gdft_memset_async(q, out->v3sigma3,     0, sizeof(double)*np*func->dim.v3sigma3);
  }
  if(order >= 4) {
    gdft_memset_async(q, out->v4rho4,       0, sizeof(double)*np*func->dim.v4rho4);
    gdft_memset_async(q, out->v4rho3sigma,  0, sizeof(double)*np*func->dim.v4rho3sigma);
    gdft_memset_async(q, out->v4rho2sigma2, 0, sizeof(double)*np*func->dim.v4rho2sigma2);
    gdft_memset_async(q, out->v4rhosigma3,  0, sizeof(double)*np*func->dim.v4rhosigma3);
    gdft_memset_async(q, out->v4sigma4,     0, sizeof(double)*np*func->dim.v4sigma4);
  }
  gdft_stream_sync(q);
}
...
extern "C" int GDFT_xc_lda(
  ...
  auto stream = gdft_stream_cast(stream_v);
  double* eps    = out->zk;
...
static void debug_dump_gga(gdft_stream_t stream, const char* tag,
  ...
  gdft_memcpy_d2h(stream, h_rho.data(),  rho,    N*sizeof(double));
  gdft_memcpy_d2h(stream, h_sig.data(),  sigma,  N*sizeof(double));
  if(eps)    gdft_memcpy_d2h(stream, h_eps.data(),  eps,    N*sizeof(double));
  if(vrho)   gdft_memcpy_d2h(stream, h_vrho.data(), vrho,   N*sizeof(double));
  if(vsigma) gdft_memcpy_d2h(stream, h_vsig.data(), vsigma, N*sizeof(double));
  gdft_stream_sync(stream);
  ...
  if(eps)    { gdft_memcpy_d2h(stream, all_eps.data(),  eps,    np*sizeof(double)); }
  if(vrho)   { gdft_memcpy_d2h(stream, all_vrho.data(), vrho,   np*sizeof(double)); }
  if(vsigma) { gdft_memcpy_d2h(stream, all_vsig.data(), vsigma, np*sizeof(double)); }
  gdft_stream_sync(stream);
