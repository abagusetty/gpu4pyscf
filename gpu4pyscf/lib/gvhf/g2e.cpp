...
#include "gint/gout2e.hpp"
#include "gint/g2e.cpp"
#include "gint/reduction.cpp"

template <int NROOTS, int GSIZE> __attribute__((always_inline))
void GINTint2e_jk_kernel(GINTEnvVars envs, JKMatrix jk, BasisProdOffsets offsets, sycl::nd_item<2>& item)
{
...
static void GINTint2e_jk_kernel0000(GINTEnvVars envs, JKMatrix jk, BasisProdOffsets offsets, sycl::nd_item<2>& item)
{
    int ntasks_ij = offsets.ntasks_ij;
    int ntasks_kl = offsets.ntasks_kl;
    int task_ij = static_cast<int>( item.get_global_id(1) );
    ...
    for (i_dm = 0; i_dm < n_dm; ++i_dm) {
        if (vj != NULL) {
            block_reduce_x<THREADSX, THREADSY>(gout0*dm[i0+nao*j0], vj+k0+nao*l0, tx, ty, item);
            block_reduce_y<THREADSX, THREADSY>(gout0*dm[k0+nao*l0], vj+i0+nao*j0, tx, ty, item);
...
__attribute__((always_inline))
static void GINTint2e_jk_kernel1000(GINTEnvVars envs, JKMatrix jk, BasisProdOffsets offsets, sycl::nd_item<2>& item)
{
    int ntasks_ij = offsets.ntasks_ij;
    int ntasks_kl = offsets.ntasks_kl;
    int task_ij = static_cast<int>( item.get_global_id(1) );
    ...
    for (i_dm = 0; i_dm < n_dm; ++i_dm) {
        if (vj != NULL) {
            ...
            block_reduce_y<THREADSX, THREADSY>(gout0*d_0, vj+(i0+0)+nao*(j0+0), tx, ty, item);
            block_reduce_y<THREADSX, THREADSY>(gout1*d_0, vj+(i0+1)+nao*(j0+0), tx, ty, item);
            block_reduce_y<THREADSX, THREADSY>(gout2*d_0, vj+(i0+2)+nao*(j0+0), tx, ty, item);
            vj += nao2;
        }
        if (vk != NULL) {
            // ijkl,jl->ik
