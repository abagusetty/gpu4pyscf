...
static int GINTfill_int3c2e_tasks(ERITensor *eri, BasisProdOffsets *offsets, GINTEnvVars *envs, cudaStream_t stream)
{
    ...
    switch (type_ijkl) {
...
#else // USE_SYCL
        // nroots = 1
        case 0: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c2e_kernel0000(*envs, *eri, *offsets); }); break;
        case 1: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c2e_kernel0010(*envs, *eri, *offsets); }); break;
        case 10: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c2e_kernel0100(*envs, *eri, *offsets); }); break;
        case 100: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c2e_kernel1000(*envs, *eri, *offsets); }); break;
        // nroots = 2
        case 2: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int2e_kernel0020(*envs, *eri, *offsets); }); break;
        case 3: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int2e_kernel0030(*envs, *eri, *offsets); }); break;
        case 101: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int2e_kernel1010(*envs, *eri, *offsets); }); break;
        case 102: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int2e_kernel1020(*envs, *eri, *offsets); }); break;
        case 110: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int2e_kernel1100(*envs, *eri, *offsets); }); break;
        case 111: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int2e_kernel1110(*envs, *eri, *offsets); }); break;
        case 200: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int2e_kernel2000(*envs, *eri, *offsets); }); break;
        case 201: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int2e_kernel2010(*envs, *eri, *offsets); }); break;
        case 210: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int2e_kernel2100(*envs, *eri, *offsets); }); break;
        case 300: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int2e_kernel3000(*envs, *eri, *offsets); }); break;
        // nroots = 3
        case 103: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int2e_kernel1030(*envs, *eri, *offsets); }); break;
        case 112: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int2e_kernel1120(*envs, *eri, *offsets); }); break;
        case 113: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int2e_kernel1130(*envs, *eri, *offsets); }); break;
        case 202: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int2e_kernel2020(*envs, *eri, *offsets); }); break;
        case 203: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int2e_kernel2030(*envs, *eri, *offsets); }); break;
        case 211: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int2e_kernel2110(*envs, *eri, *offsets); }); break;
        case 212: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int2e_kernel2120(*envs, *eri, *offsets); }); break;
        case 220: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int2e_kernel2200(*envs, *eri, *offsets); }); break;
        case 221: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int2e_kernel2210(*envs, *eri, *offsets); }); break;
        case 301: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int2e_kernel3010(*envs, *eri, *offsets); }); break;
        case 302: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int2e_kernel3020(*envs, *eri, *offsets); }); break;
        case 310: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int2e_kernel3100(*envs, *eri, *offsets); }); break;
        case 311: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int2e_kernel3110(*envs, *eri, *offsets); }); break;
        case 320: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int2e_kernel3200(*envs, *eri, *offsets); }); break;
        default: {
		...
		cgh.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) {
                  GINTfill_int3c2e_kernel(*envs, *eri, *offsets, item,
