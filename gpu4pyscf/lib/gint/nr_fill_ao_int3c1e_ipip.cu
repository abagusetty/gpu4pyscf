                                                          ...
                                                          const int n_charge_sum_per_thread, const cudaStream_t stream)
{
    ...
    switch (nrys_roots) {
    case 2: stream.parallel_for<class GINTfill_int3c1e_ipip1_charge_contracted_kernel_general2>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_ipip1_charge_contracted_kernel_general<2, GSIZE5_INT3C_1E> (output, offsets, i_l, j_l, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    case 3: stream.parallel_for<class GINTfill_int3c1e_ipip1_charge_contracted_kernel_general3>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_ipip1_charge_contracted_kernel_general<3, GSIZE6_INT3C_1E> (output, offsets, i_l, j_l, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    case 4: stream.parallel_for<class GINTfill_int3c1e_ipip1_charge_contracted_kernel_general4>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_ipip1_charge_contracted_kernel_general<4, GSIZE4_INT3C_1E> (output, offsets, i_l, j_l, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    case 5: stream.parallel_for<class GINTfill_int3c1e_ipip1_charge_contracted_kernel_general5>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_ipip1_charge_contracted_kernel_general<5, GSIZE5_INT3C_1E> (output, offsets, i_l, j_l, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    case 6: stream.parallel_for<class GINTfill_int3c1e_ipip1_charge_contracted_kernel_general6>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_ipip1_charge_contracted_kernel_general<6, GSIZE6_INT3C_1E> (output, offsets, i_l, j_l, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    default:
                                                           ...
                                                           const int n_charge_sum_per_thread, const cudaStream_t stream)
{
    ...
    const int nrys_roots = (i_l + j_l + 2) / 2 + 1;
    switch (nrys_roots) {
    case 2: stream.parallel_for<class GINTfill_int3c1e_ipvip1_charge_contracted_kernel_general2>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_ipvip1_charge_contracted_kernel_general<2, GSIZE5_INT3C_1E> (output, offsets, i_l, j_l, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    case 3: stream.parallel_for<class GINTfill_int3c1e_ipvip1_charge_contracted_kernel_general3>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_ipvip1_charge_contracted_kernel_general<3, GSIZE6_INT3C_1E> (output, offsets, i_l, j_l, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    case 4: stream.parallel_for<class GINTfill_int3c1e_ipvip1_charge_contracted_kernel_general4>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_ipvip1_charge_contracted_kernel_general<4, GSIZE4_INT3C_1E> (output, offsets, i_l, j_l, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    case 5: stream.parallel_for<class GINTfill_int3c1e_ipvip1_charge_contracted_kernel_general5>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_ipvip1_charge_contracted_kernel_general<5, GSIZE5_INT3C_1E> (output, offsets, i_l, j_l, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    case 6: stream.parallel_for<class GINTfill_int3c1e_ipvip1_charge_contracted_kernel_general6>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_ipvip1_charge_contracted_kernel_general<6, GSIZE6_INT3C_1E> (output, offsets, i_l, j_l, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    default:
        fprintf(stderr, "nrys_roots = %d out of range\n", nrys_roots);
        return 1;
    }
                                                           ...
                                                           const int n_charge_sum_per_thread, const cudaStream_t stream)
{
    ...
    const int nrys_roots = (i_l + j_l + 2) / 2 + 1;
    switch (nrys_roots) {
    case 2: stream.parallel_for<class GINTfill_int3c1e_ip1ip2_charge_contracted_kernel_general2>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_ip1ip2_charge_contracted_kernel_general<2, GSIZE5_INT3C_1E> (output, offsets, i_l, j_l, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    case 3: stream.parallel_for<class GINTfill_int3c1e_ip1ip2_charge_contracted_kernel_general3>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_ip1ip2_charge_contracted_kernel_general<3, GSIZE6_INT3C_1E> (output, offsets, i_l, j_l, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    case 4: stream.parallel_for<class GINTfill_int3c1e_ip1ip2_charge_contracted_kernel_general4>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_ip1ip2_charge_contracted_kernel_general<4, GSIZE4_INT3C_1E> (output, offsets, i_l, j_l, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    case 5: stream.parallel_for<class GINTfill_int3c1e_ip1ip2_charge_contracted_kernel_general5>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_ip1ip2_charge_contracted_kernel_general<5, GSIZE5_INT3C_1E> (output, offsets, i_l, j_l, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    case 6: stream.parallel_for<class GINTfill_int3c1e_ip1ip2_charge_contracted_kernel_general6>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_ip1ip2_charge_contracted_kernel_general<6, GSIZE6_INT3C_1E> (output, offsets, i_l, j_l, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    default:
        fprintf(stderr, "nrys_roots = %d out of range\n", nrys_roots);
        return 1;
    }
                                                           ...
                                                           const int n_pair_sum_per_thread, const cudaStream_t stream)
{
    ...
    sycl::range<2> blocks((ngrids+THREADSY-1)/THREADSY, (ntasks_ij+THREADSX-1)/THREADSX);
    switch (i_l + j_l) {
    case  0: stream.parallel_for<class GINTfill_int3c1e_ipip2_density_contracted_kernel_general0>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_ipip2_density_contracted_kernel_general< 0> (output, density, hermite_density_offsets, offsets, nprim_ij, omega, grid_points, charge_exponents); }); break;
    case  1: stream.parallel_for<class GINTfill_int3c1e_ipip2_density_contracted_kernel_general1>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_ipip2_density_contracted_kernel_general< 1> (output, density, hermite_density_offsets, offsets, nprim_ij, omega, grid_points, charge_exponents); }); break;
    case  2: stream.parallel_for<class GINTfill_int3c1e_ipip2_density_contracted_kernel_general2>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_ipip2_density_contracted_kernel_general< 2> (output, density, hermite_density_offsets, offsets, nprim_ij, omega, grid_points, charge_exponents); }); break;
    case  3: stream.parallel_for<class GINTfill_int3c1e_ipip2_density_contracted_kernel_general3>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_ipip2_density_contracted_kernel_general< 3> (output, density, hermite_density_offsets, offsets, nprim_ij, omega, grid_points, charge_exponents); }); break;
    case  4: stream.parallel_for<class GINTfill_int3c1e_ipip2_density_contracted_kernel_general4>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_ipip2_density_contracted_kernel_general< 4> (output, density, hermite_density_offsets, offsets, nprim_ij, omega, grid_points, charge_exponents); }); break;
    case  5: stream.parallel_for<class GINTfill_int3c1e_ipip2_density_contracted_kernel_general5>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_ipip2_density_contracted_kernel_general< 5> (output, density, hermite_density_offsets, offsets, nprim_ij, omega, grid_points, charge_exponents); }); break;
    case  6: stream.parallel_for<class GINTfill_int3c1e_ipip2_density_contracted_kernel_general6>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_ipip2_density_contracted_kernel_general< 6> (output, density, hermite_density_offsets, offsets, nprim_ij, omega, grid_points, charge_exponents); }); break;
    case  7: stream.parallel_for<class GINTfill_int3c1e_ipip2_density_contracted_kernel_general7>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_ipip2_density_contracted_kernel_general< 7> (output, density, hermite_density_offsets, offsets, nprim_ij, omega, grid_points, charge_exponents); }); break;
    case  8: stream.parallel_for<class GINTfill_int3c1e_ipip2_density_contracted_kernel_general8>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_ipip2_density_contracted_kernel_general< 8> (output, density, hermite_density_offsets, offsets, nprim_ij, omega, grid_points, charge_exponents); }); break;
    // Up to g + g = 8 now
    default:
