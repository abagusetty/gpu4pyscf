        ...
        if (!if_no_cart2sph) {
            ...
            for (int i_group = 0; i_group < n_l_ctr_group; i_group++) {
                ...
                switch (l_i) {
                    ...
                    case  2: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { left_sph2cart_inplace< 2> (cartesian_matrix, n_ao_cartesian, n_bas, i_cartesian_offset); }); break;
                    case  3: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { left_sph2cart_inplace< 3> (cartesian_matrix, n_ao_cartesian, n_bas, i_cartesian_offset); }); break;
                    case  4: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { left_sph2cart_inplace< 4> (cartesian_matrix, n_ao_cartesian, n_bas, i_cartesian_offset); }); break;
                    case  5: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { left_sph2cart_inplace< 5> (cartesian_matrix, n_ao_cartesian, n_bas, i_cartesian_offset); }); break;
                    case  6: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { left_sph2cart_inplace< 6> (cartesian_matrix, n_ao_cartesian, n_bas, i_cartesian_offset); }); break;
                    case  7: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { left_sph2cart_inplace< 7> (cartesian_matrix, n_ao_cartesian, n_bas, i_cartesian_offset); }); break;
                    case  8: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { left_sph2cart_inplace< 8> (cartesian_matrix, n_ao_cartesian, n_bas, i_cartesian_offset); }); break;
                    case  9: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { left_sph2cart_inplace< 9> (cartesian_matrix, n_ao_cartesian, n_bas, i_cartesian_offset); }); break;
                    case 10: stream.parallel_for(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { left_sph2cart_inplace<10> (cartesian_matrix, n_ao_cartesian, n_bas, i_cartesian_offset); }); break;
                    default:
