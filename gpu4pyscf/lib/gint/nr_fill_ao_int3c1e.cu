                                  ...
                                  const double omega, const double* grid_points, const double* charge_exponents, const cudaStream_t stream)
{
    ...
    case 1:
        type_ijkl = (i_l << 2) | j_l;
        switch (type_ijkl) {
        case (0<<2)|0: stream.parallel_for<class GINTfill_int3c1e_kernel00_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_kernel00(output, offsets, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
        case (1<<2)|0: stream.parallel_for<class GINTfill_int3c1e_kernel10_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_kernel10(output, offsets, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
        default:
            fprintf(stderr, "roots=1 type_ijkl %d\n", type_ijkl);
        }
        break;
    case 2: stream.parallel_for<class GINTfill_int3c1e_kernel_general_2_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_kernel_general<2, GSIZE2_INT3C_1E> (output, offsets, i_l, j_l, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    case 3: stream.parallel_for<class GINTfill_int3c1e_kernel_general_3_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_kernel_general<3, GSIZE3_INT3C_1E> (output, offsets, i_l, j_l, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    case 4: stream.parallel_for<class GINTfill_int3c1e_kernel_general_4_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_kernel_general<4, GSIZE4_INT3C_1E> (output, offsets, i_l, j_l, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    case 5: stream.parallel_for<class GINTfill_int3c1e_kernel_general_5_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_kernel_general<5, GSIZE5_INT3C_1E> (output, offsets, i_l, j_l, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    default:
                                                    ...
                                                    const int n_charge_sum_per_thread, const cudaStream_t stream)
{
    ...
    sycl::range<2> blocks((ngrids+THREADSY-1)/THREADSY, (ntasks_ij+THREADSX-1)/THREADSX);
    switch (type_ij) {
    case 00: stream.parallel_for<class GINTfill_int3c1e_charge_contracted_kernel00_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_charge_contracted_kernel00(output, offsets, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    case 10: stream.parallel_for<class GINTfill_int3c1e_charge_contracted_kernel10_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_charge_contracted_kernel10(output, offsets, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    case 11: stream.parallel_for<class GINTfill_int3c1e_charge_contracted_kernel_expanded_11_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_charge_contracted_kernel_expanded<1, 1> (output, offsets, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    case 20: stream.parallel_for<class GINTfill_int3c1e_charge_contracted_kernel_expanded_20_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_charge_contracted_kernel_expanded<2, 0> (output, offsets, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    case 21: stream.parallel_for<class GINTfill_int3c1e_charge_contracted_kernel_expanded_21_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_charge_contracted_kernel_expanded<2, 1> (output, offsets, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    case 22: stream.parallel_for<class GINTfill_int3c1e_charge_contracted_kernel_expanded_22_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_charge_contracted_kernel_expanded<2, 2> (output, offsets, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    case 30: stream.parallel_for<class GINTfill_int3c1e_charge_contracted_kernel_expanded_30_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_charge_contracted_kernel_expanded<3, 0> (output, offsets, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    case 31: stream.parallel_for<class GINTfill_int3c1e_charge_contracted_kernel_expanded_31_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_charge_contracted_kernel_expanded<3, 1> (output, offsets, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    case 32: stream.parallel_for<class GINTfill_int3c1e_charge_contracted_kernel_expanded_32_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_charge_contracted_kernel_expanded<3, 2> (output, offsets, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    case 40: stream.parallel_for<class GINTfill_int3c1e_charge_contracted_kernel_expanded_40_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_charge_contracted_kernel_expanded<4, 0> (output, offsets, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    case 41: stream.parallel_for<class GINTfill_int3c1e_charge_contracted_kernel_expanded_41_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_charge_contracted_kernel_expanded<4, 1> (output, offsets, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
    default:
        const int nrys_roots = (i_l + j_l) / 2 + 1;
        switch (nrys_roots) {
        case 4: stream.parallel_for<class GINTfill_int3c1e_charge_contracted_kernel_general_4_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_charge_contracted_kernel_general<4, GSIZE4_INT3C_1E> (output, offsets, i_l, j_l, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
        case 5: stream.parallel_for<class GINTfill_int3c1e_charge_contracted_kernel_general_5_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_charge_contracted_kernel_general<5, GSIZE5_INT3C_1E> (output, offsets, i_l, j_l, nprim_ij, stride_j, stride_ij, ao_offsets_i, ao_offsets_j, omega, grid_points, charge_exponents); }); break;
        default:
                                                     ...
                                                     const int n_pair_sum_per_thread, const cudaStream_t stream)
{
    ...
    sycl::range<2> blocks((ngrids+THREADSY-1)/THREADSY, (ntasks_ij+THREADSX-1)/THREADSX);
    switch (i_l + j_l) {
    case  0: stream.parallel_for<class GINTfill_int3c1e_density_contracted_kernel00_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_density_contracted_kernel00(output, density, hermite_density_offsets, offsets, nprim_ij, omega, grid_points, charge_exponents); }); break;
    case  1: stream.parallel_for<class GINTfill_int3c1e_density_contracted_kernel10_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_density_contracted_kernel10(output, density, hermite_density_offsets, offsets, nprim_ij, omega, grid_points, charge_exponents); }); break;
    case  2: stream.parallel_for<class GINTfill_int3c1e_density_contracted_kernel_general_2_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_density_contracted_kernel_general< 2> (output, density, hermite_density_offsets, offsets, nprim_ij, omega, grid_points, charge_exponents); }); break;
    case  3: stream.parallel_for<class GINTfill_int3c1e_density_contracted_kernel_general_3_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_density_contracted_kernel_general< 3> (output, density, hermite_density_offsets, offsets, nprim_ij, omega, grid_points, charge_exponents); }); break;
    case  4: stream.parallel_for<class GINTfill_int3c1e_density_contracted_kernel_general_4_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_density_contracted_kernel_general< 4> (output, density, hermite_density_offsets, offsets, nprim_ij, omega, grid_points, charge_exponents); }); break;
    case  5: stream.parallel_for<class GINTfill_int3c1e_density_contracted_kernel_general_5_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_density_contracted_kernel_general< 5> (output, density, hermite_density_offsets, offsets, nprim_ij, omega, grid_points, charge_exponents); }); break;
    case  6: stream.parallel_for<class GINTfill_int3c1e_density_contracted_kernel_general_6_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_density_contracted_kernel_general< 6> (output, density, hermite_density_offsets, offsets, nprim_ij, omega, grid_points, charge_exponents); }); break;
    case  7: stream.parallel_for<class GINTfill_int3c1e_density_contracted_kernel_general_7_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_density_contracted_kernel_general< 7> (output, density, hermite_density_offsets, offsets, nprim_ij, omega, grid_points, charge_exponents); }); break;
    case  8: stream.parallel_for<class GINTfill_int3c1e_density_contracted_kernel_general_8_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { GINTfill_int3c1e_density_contracted_kernel_general< 8> (output, density, hermite_density_offsets, offsets, nprim_ij, omega, grid_points, charge_exponents); }); break;
    // Up to g + g = 8 now
    default:
