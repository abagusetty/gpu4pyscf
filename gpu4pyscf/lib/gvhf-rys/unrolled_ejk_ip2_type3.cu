                            ...
                            )
{
    ...
    int thread_id = threadIdx_y * nsq_per_block + threadIdx_x;
    int threads = nsq_per_block * gout_stride;
    int *bas_kl_idx = pool + blockIdx_x * QUEUE_DEPTH;
while (1) {
    if (thread_id == 0) {
                        ...
                        int *pool, double *dd_pool)
{
    ...
    switch (ijkl) {
    case 0:
         stream.submit([&](sycl::handler &cgh) { sycl::local_accessor<double, 1> local_acc(sycl::range<1>(buflen), cgh); cgh.parallel_for<class rys_ejk_ip2_type3_0000_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { rys_ejk_ip2_type3_0000(dev_envs, dev_jk, dev_bounds, pool, dd_pool, head, item, GPU4PYSCF_IMPL_SYCL_GET_MULTI_PTR(local_acc)); }); }); break;
    case 125:
        stream.submit([&](sycl::handler &cgh) { sycl::local_accessor<double, 1> local_acc(sycl::range<1>(buflen), cgh); cgh.parallel_for<class rys_ejk_ip2_type3_1000_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { rys_ejk_ip2_type3_1000(dev_envs, dev_jk, dev_bounds, pool, dd_pool, head, item, GPU4PYSCF_IMPL_SYCL_GET_MULTI_PTR(local_acc)); }); }); break;
    case 130:
        stream.submit([&](sycl::handler &cgh) { sycl::local_accessor<double, 1> local_acc(sycl::range<1>(buflen), cgh); cgh.parallel_for<class rys_ejk_ip2_type3_1010_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { rys_ejk_ip2_type3_1010(dev_envs, dev_jk, dev_bounds, pool, dd_pool, head, item, GPU4PYSCF_IMPL_SYCL_GET_MULTI_PTR(local_acc)); }); }); break;
    case 131:
        buflen += (bounds->g_size * 3 + 9) * nsq_per_block;
        stream.submit([&](sycl::handler &cgh) { sycl::local_accessor<double, 1> local_acc(sycl::range<1>(buflen), cgh); cgh.parallel_for<class rys_ejk_ip2_type3_1011_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { rys_ejk_ip2_type3_1011(dev_envs, dev_jk, dev_bounds, pool, dd_pool, head, item, GPU4PYSCF_IMPL_SYCL_GET_MULTI_PTR(local_acc)); }); }); break;
    case 150:
        stream.submit([&](sycl::handler &cgh) { sycl::local_accessor<double, 1> local_acc(sycl::range<1>(buflen), cgh); cgh.parallel_for<class rys_ejk_ip2_type3_1100_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { rys_ejk_ip2_type3_1100(dev_envs, dev_jk, dev_bounds, pool, dd_pool, head, item, GPU4PYSCF_IMPL_SYCL_GET_MULTI_PTR(local_acc)); }); }); break;
    case 155:
        buflen += (bounds->g_size * 3 + 9) * nsq_per_block;
        stream.submit([&](sycl::handler &cgh) { sycl::local_accessor<double, 1> local_acc(sycl::range<1>(buflen), cgh); cgh.parallel_for<class rys_ejk_ip2_type3_1110_sycl>(sycl::nd_range<2>(blocks * threads, threads), [=](auto item) { rys_ejk_ip2_type3_1110(dev_envs, dev_jk, dev_bounds, pool, dd_pool, head, item, GPU4PYSCF_IMPL_SYCL_GET_MULTI_PTR(local_acc)); }); }); break;
    case 156:
        buflen += (bounds->g_size * 3 + 9) * nsq_per_block;
